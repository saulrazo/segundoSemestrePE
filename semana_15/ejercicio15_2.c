...
{   
    ...
    for (cont = 0; cont < strlen(s1) / 2; cont++)
    {
        aux = s1[cont]; //Guarda el valor a intercambiar en una variable auxiliar
        s1[cont] = s1[lenght - cont - 1]; //Realiza el cambio en cuyo caso de la primera con la última, segunda con penúltima...
        s1[lenght - cont - 1] = aux; //Asigna en cuyo caso al último espacio el valor del auxiliar que correspondía al otro extremo de la palabra.
    }
}
